<div class="user media user-mini notification {{$notification->id}}-notification">
      ...
      <div class="media-body">
           <h5 class="media-heading">
               ...
               <a href="{{$notification->user->present()->url()}}">{{$notification->user->fullname}}</a> {{trans('notification.added-comment')}}
               <?php $photo = app('App\\Repositories\\PhotoRepository')->get($typeId)?>
               @if($photo)
                <a class="preview-image" rel="album" href="{{Image::url($photo->path, 'original')}}">{{trans('photo.photo')}}</a>
               @endif
           </h5>
